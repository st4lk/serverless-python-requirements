...
function getFunctionShortName(funcName, serverless) {
  return Object.keys(serverless.service.functions)
    .find(key => serverless.service.functions[key].name === funcName);
}
  ...
  if (this.serverless.service.package.individually) {
    return BbPromise.resolve(values(this.serverless.service.functions))
      ...
      .filter(func => {
        ...
        return !cliOptions.function || cliOptions.function === getFunctionShortName(
          func.name,
          this.serverless
        );
      })
